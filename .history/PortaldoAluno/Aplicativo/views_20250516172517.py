...
def add_eventos(request):
    ...
    if request.method == 'POST':
        ...
        if nome and data:
            EventoCalendario.objects.create(nome=nome, data=data, horario=descricao, user=request.user)
            from django.contrib import messages
...
@login_required
def desempenho_frequencia_view(request, aluno_id):
    desempenho = get_object_or_404(DesempenhoFrequencia, aluno__id=aluno_id)
